...
"""Student one-step plus rollout loss with curriculum weighting."""
...
def rollout_loss(model, states, actions, normalizer, warmup_steps, horizon):
    ...
    # 后期步骤权重更大，鼓励长期准确
    T = pred_norm.shape[1]
    weights = torch.linspace(1.0, 3.0, T, device=pred_norm.device)
...
def compute_loss(model, batch, normalizer, cfg):
    ...
    horizon = int(loss_cfg.get("rollout_train_horizon", 40))
